...
- 🖥️ **Web Terminal** - 基于 xterm.js 的浏览器 SSH 终端，支持完整的终端交互与尺寸实时同步
- 📹 **会话录像回放** - 使用 asciinema-player 进行会话录像的在线回放与下载
- 📁 **SFTP 文件管理** - 浏览器内远程文件浏览、上传、下载、删除、重命名、新建目录
- 🚀 **任务中心** - 支持快速命令/脚本/文件分发、任务模板、可视化任务编排与批量执行
- 🛡️ **高危指令拦截** - 实时危险命令拦截提示（Web Terminal 与 SSH 堡垒机双通道）
- 🎨 **现代化 UI** - 基于 Element Plus 2.x 的美观界面设计，4 种布局模式
- 🔐 **权限控制** - 精细的按钮级权限控制（`v-auth` / `v-auths` / `v-auth-all`）
- 📱 **响应式设计** - 适配桌面、平板等多种设备
- ⚡ **高性能** - Vite 构建，支持热更新、代码分割、Gzip 压缩
- 🌍 **国际化** - 内置 i18n 多语言支持
...
> **注意**：本项目使用 **Vite 默认端口 `1080`** 作为开发服务器端口（非 3000），代理配置自动转发 `/api` 到后端 `localhost:8080`。
...
启动成功后，浏览器会自动打开 `http://localhost:1080`
...
│   │   ├── script/           # 脚本库接口
│   │   ├── sftp/             # SFTP 文件管理接口
│   │   ├── taskExecution/    # 任务执行接口
│   │   ├── taskPipeline/     # 任务编排接口
│   │   ├── taskTemplate/     # 任务模板接口
│   │   ├── terminal/         # 终端相关接口
│   │   ├── terminal/record.ts # 录像回放接口
│   │   └── user/             # 用户管理接口
│   ├── assets/               # 资源文件
│   ├── components/           # 公共组件
│   │   ├── auth/             # 权限组件
...
│   ├── views/                # 页面组件
│   │   ├── auth/             # 授权管理页面
│   │   ├── dangerousCommand/ # 高危指令管理页面
│   │   ├── group/            # 主机分组页面
│   │   ├── instance/         # 主机管理页面（含终端、SFTP、绑定凭证）
│   │   ├── keys/             # 凭证管理页面
│   │   ├── myinstance/       # 我的主机页面
│   │   ├── script/           # 脚本库页面
│   │   ├── system/           # 系统管理页面（用户/角色/菜单/部门/日志）
│   │   ├── taskExecution/    # 任务执行记录页面
│   │   ├── taskPipeline/     # 任务编排页面
│   │   ├── taskTemplate/     # 任务模板页面
│   │   ├── terminal/         # Web Terminal 全屏终端与录像回放
│   │   └── ...               # 其他演示与工具页面
│   ├── App.vue               # 根组件
│   └── main.ts               # 入口文件
├── .env                      # 环境变量
...
检查端口 `1080` 是否被占用，修改 `.env` 中的 `VITE_PORT`。

### 3. WebSocket 连接失败

检查 Vite 代理配置，确保 `ws: true` 已开启。Vite 配置文件中 `/api` 代理已默认开启 WebSocket 支持。

### 4. 终端无法连接

- 检查后端服务是否启动在 `8080` 端口
- 确认 Vite 代理配置正确（开发环境）
- 生产环境确认 Nginx 已配置 WebSocket 转发
- 查看浏览器控制台错误信息
